...
def get_sinogram(phantom, deltarho, numrhos, angles):
    proj_geom =astra.create_proj_geom('parallel', deltarho, numrhos, angles)
...
def get_sinogram3D(phantom, numrhos, angles):
    proj_geom =astra.create_proj_geom('parallel3d', 1.0, 1.0, phantom.shape[0], numrhos, angles)
    # proj_geom =astra.create_proj_geom('parallel3d', deltarho, deltarho, phantom.shape[0], numrhos, angles)
    # proj_geom =astra.create_proj_geom('parallel3d', deltarho, 1.0, phantom.shape[0], numrhos, angles)
...
def get_angles(size, degrees = False, num_angles = 0):
    num_angles = math.ceil(size*math.pi/2) if num_angles == 0 else num_angles
    if degrees:
...
def savebackprojection(filename, data, size):
    reshaped = np.flip(data,0).reshape((size,size))
    scipy.misc.toimage(reshaped).save(filename)
...
def get_phantom(size):
    ...
    return (np.where(x**2+y**2 <= r**2, full_image,0.0), full_image)
...
def get_phantom3D(size):
    ...
    x = round(size/3)
    y = round(size/10)
    cube[x:size-x,x:size-x,x:size-x] = 1
    cube[x+y:size-x+y,x+y:size-x+y,:] = 0
    return cube;
