...
def plot(directory,title,xlabel,ylabel):
    ...
    fut_bp_x, fut_bp_y = np.loadtxt(os.path.join(root,"fut_bp.csv"), skiprows=0, delimiter=';', unpack=True)
    # astra_fp_x, astra_fp_y = np.loadtxt(os.path.join(root,"astra_fp.csv"), skiprows=0, delimiter=';', unpack=True)
    astra_bp_x, astra_bp_y = np.loadtxt(os.path.join(root,"fut_bp_excess.csv"), skiprows=0, delimiter=';', unpack=True)
    ...
    plt.legend(["no excess", "excess parallelism"])
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
