...
def plot(directory,title,xlabel,ylabel):
    ...
    fut_fp_x, fut_fp_y = np.loadtxt(os.path.join(root,"fut_fp_nodiv.csv"), skiprows=0, delimiter=';', unpack=True)
    fut_bp_x, fut_bp_y = np.loadtxt(os.path.join(root,"fut_bp_nodiv.csv"), skiprows=0, delimiter=';', unpack=True)
    astra_fp_x, astra_fp_y = np.loadtxt(os.path.join(root,"fut_fp_div.csv"), skiprows=0, delimiter=';', unpack=True)
    astra_bp_x, astra_bp_y = np.loadtxt(os.path.join(root,"fut_bp_div.csv"), skiprows=0, delimiter=';', unpack=True)
